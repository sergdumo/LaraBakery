...
- **Dashboard** (`/admin`): selector `Mes | Año`. La vista mensual conserva ventas, ganancia estimada, ticket promedio, pedidos, cobros, costos, gráfica diaria y distribución interactiva. La vista anual agrega indicadores acumulados, ventas vs. ganancia por mes, navegación al detalle mensual, curva acumulada, rankings de productos y clientes por facturación, cobertura de costos y mapa diario de actividad; excluye cancelados y no inventa metas ni comparaciones sin historial. El ranking de clientes consolida variaciones de mayúsculas, tildes y espacios del nombre, sin mostrar datos de contacto.
